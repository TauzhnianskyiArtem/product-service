...
public class ApplyDiscountOneProductStep {
     ...
     *
     * @param productId of the product to be applied discount
     * @param dataTable - discountRequestDto parameters in feature file
     */
    @When("a client wants apply discount for product with id {long}:")
    public void aClientWantsApplyDiscountForProduct(long productId, DataTable dataTable) {
        final DiscountRequestDto request = ProductModelMapper.toDiscountRequestDto(dataTable);
        final var response = HttpClientExceptionHandler.sendRequest(
                () -> productControllerApi.applyDiscountWithHttpInfo(productId, request),
